...
public class Rot47x {	
	int chave = 0;
	public String getCifra(String mensagem_original, int chave)
	{
		this.chave = chave;
		...
		for(int i=0; i < mensagem_original.length();i++)
		{
			...
			{
				valor_ascii -= 33;
				novo_valor_ascii = (valor_ascii + this.chave) % 94;
	...
	public String getDeCifra(String mensagem_cifrada, int chave){
		
		this.chave = chave;
		int novo_valor_ascii;
		int valor_ascii;
		String mensagem_original = "";
		//percorra o meu texto cifrado
		for(int i=0; i < mensagem_cifrada.length();i++)
		{
			...
			{
				valor_ascii -= 33;
				novo_valor_ascii = ((valor_ascii - this.chave) + 94)  % 94;
